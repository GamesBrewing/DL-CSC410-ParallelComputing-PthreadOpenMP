...
void update_bodies(Body bodies[], int num_bodies, double dt, int start, int end) {
    double fx, fy;
...
int main() {
    ...
    // Initializing position, velocity, and mass for each body. Only do this on the first rank.
    if (rank == 0){
        for (int i = 0; i < NUM_BODIES; i++) {
    ...
    for (int step = 0; step < 1000; step++) {
        ...
        update_bodies(bodies, NUM_BODIES, DT, start, end);

        MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, bodies, chunk_size * sizeof(Body), MPI_BYTE, MPI_COMM_WORLD);    
        }
